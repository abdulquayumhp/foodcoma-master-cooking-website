...
const MyReview = () => {
	useTitle("My Review");
	const { userInfo, loading, setLoading, UserLogOut } = useContext(UserContext);
	...
	useEffect(() => {
		fetch(
			` https://server-gray-tau.vercel.app/myReview?email=${userInfo?.email}`,
			{
				headers: {
	...
	}, [userInfo?.email, UserLogOut, reviewData]);
